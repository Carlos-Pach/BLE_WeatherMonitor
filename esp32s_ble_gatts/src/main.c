    ...
    switch (event) {
        ...
        case ESP_GATTS_WRITE_EVT:
            if (!param->write.is_prep){
                ...
                // add updates for temperature and humidity w/ notifications
                if ((weather_handle_table[IDX_CHAR_CFG_A] == param->write.handle) && (param->write.len == 2)){
                    ...
                    if (descr_value == 0x0001){
                        ...
                        esp_ble_gatts_send_indicate(gatts_if, param->write.conn_id, weather_handle_table[CHAR_A_VAL],
                                                sizeof(tempInC), (uint8_t*)&tempInC, false);
                ...
                } else if ((weather_handle_table[IDX_CHAR_CFG_B] == param->write.handle) && (param->write.len == 2)){
                    uint16_t descr_value = param->write.value[1]<<8 | param->write.value[0];
                    if (descr_value == 0x0001){
                        ESP_LOGI(GATTS_TABLE_TAG, "notify enable");
                        esp_ble_gatts_send_indicate(gatts_if, param->write.conn_id, weather_handle_table[CHAR_B_VAL],
                                                sizeof(humidityBLE), (uint8_t*)&humidityBLE, false);
                    } else if (descr_value == 0x0002){
                        ESP_LOGI(GATTS_TABLE_TAG, "indicate enable");
                        uint8_t indicate_data[15];
                        for (int i = 0; i < sizeof(indicate_data); ++i)
